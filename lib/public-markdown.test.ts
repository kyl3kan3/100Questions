...
describe("public Markdown editions", () => {
  ...
  it("returns negotiation headers and no body for HEAD", async () => {
    const page = publicMarkdownForHtmlPath("/about")!;
    const response = publicMarkdownResponse(page);
    expect(response.headers.get("content-type")).toContain("text/markdown");
    expect(response.headers.get("content-location")).toBe("/about.md");
    expect(response.headers.get("vary")).toBe("Accept");
    expect(response.headers.get("link")).toContain(
      '<https://100questionsai.com/about>; rel="canonical"',
    );
    expect(await response.text()).toContain("# About 100 Questions");

    const head = publicMarkdownResponse(page, "HEAD");
    expect(await head.text()).toBe("");
  });

  it("lists every exact Markdown URL in llms.txt", () => {
    const llms = readFileSync("public/llms.txt", "utf8");

    for (const page of ALL_PUBLIC_MARKDOWN_PAGES) {
